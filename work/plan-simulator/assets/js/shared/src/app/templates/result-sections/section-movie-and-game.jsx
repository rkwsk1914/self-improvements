...
/* ---------------------------------------------------------------------------------
  明細エリア 動画サービス / ゲームセクション
--------------------------------------------------------------------------------- */
export class SectionMovieAndGame extends Component {
  /**
   *オブジェクトの生成
  ...
  createInput () {
    ...
    for (let index = 0; index < selectionKeys.length; index++) {
      ...
      const selectionDom = (
        <div className="simulator-modal__card__body__detail-section__check-box column-2" key={index}>
          <CheckBox
            data={checkBoxData}
            id={selectionId}
            ctrData={ctrData}
